...
| D05 | §4.2 | *Applied, superseding the earlier "keep as written".* Step 4(a) now prefers, among windows that pass the check digit, one spanning a whole run, then one aligned to a run's start, then its end. Verified against every fixture with no regression and no increase in false extraction. Step 1 still joins runs across whitespace; that is deliberate, since it rejoins the grouped display form, and is pinned as a documented-behaviour test. |
| D17 | §4.3, §6.3, §6.4 | *Applied.* **A mismatch only blocks where a check digit exists.** `checkDigitApplies(vin)` is true when position 9 is a digit or `X`; a letter there means the identifier carries no ISO 3779 check digit. Where it does not apply the scan proceeds normally with a neutral note and no banner. `checkDigitValid` is still computed and stored for every VIN, so §5.1, §5.2 and the §4.12 column are unchanged. Without this, vehicles never sold in North America and off-highway machine PINs are told on every scan that the read is probably wrong. |
| D18 | §4.5 | *Applied.* The WMI seed candidate list now spans heavy trucks, pickups and vans, passenger cars, imports, motorcycles and trailers. Unresolved candidates are dropped by the script, so breadth costs one API call each and nothing else. |
| D19 | §4.2 | Documented limit, no code: an identifier with no check digit can only be located when it is a run of its own. That covers door-jamb barcodes and the `I`-prefixed form. Embedded in a longer undelimited run it yields `NO_VIN`, because nothing distinguishes it from its neighbours. |
...
- §4.1 requires exactly 17 characters, so pre-1981 vehicles and equipment with shorter serials cannot be entered at all. A deliberate limit, not a defect; changing it is a scope decision.
- §4.6 enables Code 39, Code 128, Data Matrix and QR only. Whether any label class in this fleet uses another symbology is a question the S1 bench can answer with real labels.
